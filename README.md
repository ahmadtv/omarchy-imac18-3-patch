...
- 🌗 **Skewed Apple logo on *warm* reboots** with 5K active — root-caused to the display never being turned off at reboot, so Apple's firmware inherited a live dual-tile panel. Fix: shut the display down properly at reboot, undo every register the stitch wrote into the second tile, and power the panel off before the handoff (`patches/5k-latch-clear.patch`). **Straight logo confirmed on hardware once**; behind its own boot entry until promoted.
- 🪞 **Sheared seam after login** — the driver's master pick left the slave tile out of the hardware sync group (fixed, shipped), and on a full modeset the one-shot CRTC alignment ran before the re-trained tile was up, so the seam could still shear right after login. A settle-and-resync 250 ms after each tiled commit is in `patches/5k-genlock-settle-resync.patch`, experimental, in the test entry only. Until it ships: any later modeset re-syncs (toggle `bitdepth` in `monitors.lua` and `hyprctl reload`).
